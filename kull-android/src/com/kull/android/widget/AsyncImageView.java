...
public class AsyncImageView extends ImageView implements ImageRequestCallback {
    ...
    private boolean mPaused,cache2File=false;
    ...
    public boolean isCache2File() {
		return cache2File;
	}

	public void setCache2File(boolean cache2File) {
    ...
    public void reload(boolean force) {
        if (mRequest == null && mUrl != null) {
            ...
            mBitmap = null;
            if (!force) {
                mBitmap = KullApplication.getImageCache(getContext()).get(mUrl);
            }
            ...
            if (KullApplication.CONFIG_INFO_LOGS_ENABLED) {
                Log.i(LOG_TAG, "Cache miss. Starting to load the image at the given URL");
            }

            setDefaultImage();
            mRequest = new ImageRequest(mUrl,isCache2File(), this, mImageProcessor, mOptions);

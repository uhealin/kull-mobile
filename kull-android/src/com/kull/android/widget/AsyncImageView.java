...
import com.kull.android.Utils;
import com.kull.android.image.ImageProcessor;
import com.kull.android.image.ImageRequest;
import com.kull.android.image.ImageRequest.ImageRequestCallback;
...
public class AsyncImageView extends ImageView implements ImageRequestCallback {
    ...
    public AsyncImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, RHelper.styleable.kull_AsyncImageView, defStyle, 0);

        Drawable d = a.getDrawable(RHelper.styleable.kull_AsyncImageView_defaultSrc);
        if (d != null) {
            setDefaultImageDrawable(d);
        }

        final int inDensity = a.getInt(RHelper.styleable.kull_AsyncImageView_inDensity, -1);
        if (inDensity != -1) {
            setInDensity(inDensity);
        }

        setUrl(a.getString(RHelper.styleable.kull_AsyncImageView_url));
    ...
    public void reload(boolean force) {
        if (mRequest == null && mUrl != null) {
            ...
            mBitmap = null;
            if (!force) {
                mBitmap = Utils.getImageCache(getContext()).get(mUrl);
            }

...
import com.kull.android.RHelper.RType;
...
public class ActionBar extends LinearLayout {
    ...
    public ActionBar(Context context, AttributeSet attrs) {
        this(context, attrs,RHelper.attr.kull_ActionBarStyle);
    }
    ...
    public ActionBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, RHelper.styleable.kull_ActionBar, defStyle, 0);

        mTitle = a.getString(RHelper.styleable.kull_ActionBar_title);

        mDividerDrawable = a.getDrawable(RHelper.styleable.kull_ActionBar_dividerDrawable);
        mDividerWidth = a.getDimensionPixelSize(RHelper.styleable.kull_ActionBar_dividerWidth, -1);
        mHomeDrawable = a.getDrawable(RHelper.styleable.kull_ActionBar_homeDrawable);
        mMaxItemsCount = a.getInt(RHelper.styleable.kull_ActionBar_maxItems, 3);
        if (mHomeDrawable == null) {
            mHomeDrawable = new ActionBarDrawable(context, RHelper.drawable.kull_action_bar_home);
        }

        int layoutID;
        int type = a.getInteger(RHelper.styleable.kull_ActionBar_type, -1);
        switch (type) {
            case 2:
                mType = Type.Empty;
                layoutID = RHelper.layout.kull_action_bar_empty;
                break;
            case 1:
                mType = Type.Dashboard;
                layoutID = RHelper.layout.kull_action_bar_dashboard;
                break;
            case 0:
            default:
                mType = Type.Normal;
                layoutID = RHelper.layout.kull_action_bar_normal;
                break;
        }
    ...
    protected void onFinishInflate() {
        ...
        if (!mMerging) {
            ...
            switch (mType) {
                case Dashboard:
                    mHomeButton = (ImageButton) findViewById(RHelper.id.kull_action_bar_home_item);
                    mHomeButton.setOnClickListener(mClickHandler);
                    break;

                case Empty:
                    mTitleView = (TextView) findViewById(RHelper.id.kull_action_bar_title);
                    setTitle(mTitle);
                    break;
                ...
                default:
                    ...
                    mHomeButton.setContentDescription(getContext().getString(RHelper.string.kull_go_home));
                    mTitleView = (TextView) findViewById(RHelper.id.kull_action_bar_title);
                    setTitle(mTitle);
                    break;
    ...
    public ActionBarItem addItem(ActionBarItem item, int itemId) {
        ...
        if (item != null) {
            ...
            itemView.findViewById(RHelper.id.kull_action_bar_item).setOnClickListener(mClickHandler);

            final int size = (int) getResources().getDimension(RHelper.dimen.kull_action_bar_height);
        ...
        public void onClick(View v) {
            if (mOnActionBarListener != null) {
                ...
                for (int i = 0; i < itemCount; i++) {
                    ...
                    final View itemButton = item.getItemView().findViewById(RHelper.id.kull_action_bar_item);
                    if (v == itemButton) {
                        item.onItemClicked();

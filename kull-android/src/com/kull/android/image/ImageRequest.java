...
import com.kull.android.image.ImageLoader.ImageLoaderCallback;
...
public class ImageRequest {
    ...
    public ImageRequest(String url,boolean cache2File, ImageRequestCallback callback) {
        this(url,cache2File, callback, null);
    }

    public ImageRequest(String url,boolean cache2File, ImageRequestCallback callback, ImageProcessor bitmapProcessor) {
        this(url,cache2File, callback, bitmapProcessor, null);
    }

    public ImageRequest(String url,boolean cache2File, ImageRequestCallback callback, ImageProcessor bitmapProcessor, BitmapFactory.Options options) {
        mUrl = url;
        mCallback = callback;
        mBitmapProcessor = bitmapProcessor;
        mOptions = options;
        this.cache2File=cache2File;
    }
    ...
    public void load(Context context) {
        if (mFuture == null) {
            ...
            mFuture = sImageLoader.loadImage(mUrl,cache2File, new InnerCallback(), mBitmapProcessor, mOptions);
        }
    }
